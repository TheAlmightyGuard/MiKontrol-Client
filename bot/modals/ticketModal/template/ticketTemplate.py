    ...
    async def close_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if agent == 0:
            if interaction.user.guild_permissions.administrator:
                ...
                await interaction.response.send_message(f"{interaction.user.mention} You do not have permission to close unclaimed tickets!", delete_after=5, ephemeral=True)
                return
        else:
        ...
        if authorized:
            ...
            await interaction.response.send_message(f"Agent {interaction.user.mention} ({interaction.user.id}) has closed this ticket. Closing <t:{round(add_time("5s", datetime.now()).timestamp())}:R>", delete_after=5)
